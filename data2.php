...
$resulta = mysql_query("insert into log values('0','".$username." inserts data into Employee database.PF No:".$emp."','".$username."','".date('YmdHi')."','".date('H:i')."','".date('d/m/Y')."','1')");	
									...
									if($a==1){
                                        ...
                                        }else{echo'<script>setTimeout(function() {seeemp(5);},500);</script>';}
										exit;
									}
									else{
...
$resulta = mysql_query("insert into log values('0','".$username." disciplines employee .PF No:".$_GET['emp']."','".$username."','".date('YmdHi')."','".date('H:i')."','".date('d/m/Y')."','1')");	
				...
				if($leaveac<$diff){
						echo '<div class="alert alert-danger alert-dismissable">
							<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
						  Leave application not succesful. You have only '.$leaveac.' days left in your leave account.</div>
					  </div>';
					  exit;
				}
				...
				$resulta = mysql_query("insert into leaves values('0','".$_GET['emp']."','".$name."','".stripslashes($row['branch'])."','".stripslashes($row['position'])."','".$_GET['from']."','".$_GET['to']."','".$diff."','".$_GET['shadow']."','".$_GET['reason']."','".date('d/m/Y')."','".date('Ymd')."',0,'".$username."','".getstamp($_GET['from'])."','".getstamp($_GET['to'])."')") or die (mysql_error());
				...
				$resultm = mysql_query("update employee set leaveac='".$leave."' where emp='".$emp."'");	
								...
								//check if employeee is on suspended leave
									...
									if($count==0){
										...
										*/
										...
										$row=mysql_fetch_array($result);
										$resultc = mysql_query("insert into payroll values('0','".$mon."','".$emp."','".$names."','".stripslashes($rowb['dept'])."','".stripslashes($rowb['branch'])."','".stripslashes($row['sal'])."','".stripslashes($row['hallow'])."','','','".stripslashes($row['car'])."','','','','','','','','','".stripslashes($row['insamount'])."','".stripslashes($row['insrelief'])."','','','','','','','','".stripslashes($row['helb'])."','".stripslashes($row['pfund'])."','','','','','','',0,'".stripslashes($rowb['bid'])."','".stripslashes($rowb['bname'])."','".stripslashes($rowb['branchname'])."','".stripslashes($rowb['eftcode'])."','".stripslashes($rowb['acno'])."','".$q."','".$monstamp."')");
...
$resultc = mysql_query("insert into benefits values('0','".$emp."','".$names."','','','','','','','','','1')");				
	...
	$('#total' + '".$emp."').val('".$total."');
		...
		$resulta = mysql_query("insert into log values('0','".$username." updates company details.','".$username."','".date('YmdHi')."','".date('H:i')."','".date('d/m/Y')."','1')");	
							...
							$result = mysql_query("insert into users values('".$user."','".$pos."','images/users/".$emp.".jpg','".$pass."','".$dept."','".$name."','".$emp."','".$bra."','')");		
							if($result){
		$resulta = mysql_query("insert into log values('0','".$username." inserts new User into System.User Id:".$user."','".$username."','".date('YmdHi')."','".date('H:i')."','".date('d/m/Y')."','1')");	
				...
				$result = mysql_query("update users set position='".$pos."',dept='".$dept."',branch='".$bra."',pfno='".$emp."',fullname='".$name."' where auto='".$auto."'");
